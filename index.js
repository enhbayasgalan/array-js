...
// > console.log(minMaxLengthAverage([7, 13, 3, 77, 100]));
// > [ 3, 100, 5, 40 ]\
function minMaxLengthAverage(sortedArray){
  let baga = sortedArray[0];
  let urt = sortedArray[0];
  let sum= 0;
  for(let i=0; i<sortedArray.length; i++){
    if(sortedArray[i] < baga)  baga = sortedArray[i]
    if(sortedArray[i] > urt )  urt = sortedArray[i]
    sum += sortedArray[i]
  }
  let avarge = sum / sortedArray.length
  //  Output = [];
  // O.push([baga, urt, sum, sortedArray.length])
  // return Ouput
  console.log(baga, urt, sortedArray.length, avarge);
}
console.log(minMaxLengthAverage([7, 13, 3, 77, 100]))
